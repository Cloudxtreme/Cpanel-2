...
class Cpanel extends Module {
	...
	public function getClientTabs($package) {
		return array(
			...
			'tabClientPass' => Language::_("Cpanel.tab_client_password", true) ,
	 ...
	 * @return string The string representing the contents of this tab
	 */
	public function tabClientPass($package, $service, array $get=null, array $post=null, array $files=null) {
		$this->view = new View("tab_client_password", "default");
		...
		if (!empty($post)) {
			...
			// Update password (if changed)
			if (!$this->Input->errors() && isset($vars['cpanel_password']) && isset($vars['cpanel_confirm_password'])) {
				
				$this->log($row->meta->host_name . "|passwd", "***", "input", true);
				$result = $this->parseResponse($api->passwd($service_fields->cpanel_username, $vars['cpanel_password']));
				Loader::loadModels($this, array("Services"));
				$this->Services->edit($service->id, array('cpanel_password' => $post['cpanel_password']));
				
				if (($errors = $this->Services->errors()))
					$this->Input->setErrors($errors);
				
				if (!$this->Input->errors())
					$service_fields->cpanel_password = $vars['cpanel_password'];
			}		
		}
	...
	/**
	 * Log In to Cpanel / Webmail / PhpMyadmin 
	 *
	 * @param stdClass $package A stdClass object representing the current package
	 * @param stdClass $service A stdClass object representing the current service
	 * @param array $get Any GET parameters
